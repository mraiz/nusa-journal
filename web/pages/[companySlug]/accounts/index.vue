  ...
  <NuxtLayout name="dashboard">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      ...
      <div class="flex gap-2">
        <button class="btn btn-secondary flex items-center gap-2" @click="handleExport" :disabled="exportLoading">
    ...
    <div class="glass-card overflow-hidden">
      ...
      <!-- Table Header -->
      <div class="grid grid-cols-12 gap-4 px-6 py-3 bg-neutral-50/80 border-b border-neutral-200 text-xs font-bold text-neutral-500 uppercase tracking-wider">
      ...
      <div v-else-if="accountsList.length === 0" class="p-12 text-center">
        ...
        <p class="text-neutral-500 mb-6 max-w-sm mx-auto">
          {{ searchQuery ? 'Tidak ditemukan akun dengan kata kunci tersebut.' : 'Mulai atur struktur keuangan perusahaan Anda dengan membuat akun pertama.' }}
        </p>
      ...
      <div v-else class="divide-y divide-neutral-100">
        <div v-for="account in accountsList" :key="account.id" 
          ...
          <div class="col-span-4 flex items-center">
             ...
             <span v-if="account.parentId" class="ml-2 text-xs text-neutral-400 bg-neutral-100 px-1 rounded">
               Sub
             </span>
          </div>

          <!-- Code -->
          <div class="col-span-2 font-mono text-sm text-neutral-600 bg-neutral-100 px-2 py-0.5 rounded w-fit">
            {{ account.code }}
          </div>
          ...
          <div class="col-span-2 text-center">
            <span v-if="account.isPosting" class="badge bg-green-100 text-green-700 border-green-200">Posting</span>
            <span v-else class="badge bg-neutral-100 text-neutral-600 border-neutral-200">Header</span>
          </div>

          <!-- Actions -->
          <div class="col-span-2 flex justify-end gap-2 opacity-100 transition-opacity">
            <button class="p-1.5 hover:bg-primary-100 text-primary-600 rounded-lg transition-colors" title="Tambah Sub-akun" @click="addChild(account)">
              ➕
            </button>
            <button class="p-1.5 hover:bg-neutral-100 text-neutral-600 rounded-lg transition-colors" title="Edit" @click="editAccount(account)">
              ✏️
            </button>
            <button class="p-1.5 hover:bg-red-100 text-red-500 rounded-lg transition-colors" title="Hapus" @click="deleteAccount(account)">
              🗑️
            </button>
          </div>
        </div>
...
const handleSave = async (payload: any) => {
  try {
    ...
    alert('Gagal menyimpan akun: ' + error)
  }
}
