  ...
  <NuxtLayout name="dashboard">
    ...
    <div class="flex items-center justify-between mb-8">
      ...
      <div class="flex gap-2">
        ...
        <button class="btn btn-primary min-w-[120px]" @click="saveJournal" :disabled="!isValid || journalStore.loading">
    ...
    <div class="space-y-6 animate-slide-up">
      ...
      <div class="glass-card p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div class="col-span-2 space-y-1">
          ...
          <input type="text" v-model="form.description" class="input py-2" placeholder="Contoh: Penyesuaian stok opname bulan Januari">
        </div>
      </div>

      <!-- Journal Lines -->
      <div class="glass-card overflow-hidden">
        <div class="p-4 bg-neutral-50/50 border-b border-neutral-100 flex justify-between items-center">
        ...
        <div class="overflow-x-auto">
          <table class="w-full">
            ...
            <tbody class="divide-y divide-neutral-100">
              <tr v-for="(line, index) in form.lines" :key="index" class="group hover:bg-neutral-50/50 transition-colors">
                <!-- Account Selector -->
                <td class="px-4 py-2">
                  <select v-model="line.accountId" class="input py-1.5 text-sm" :class="{'border-red-300': !line.accountId && attemptedSave}">
                ...
                <!-- Description -->
                <td class="px-4 py-2">
                  <input type="text" v-model="line.description" class="input py-1.5 text-sm" placeholder="Opsional">
                </td>
                ...
                <td class="px-4 py-2">
                  <div class="relative">
                    ...
                    <input type="number" v-model.number="line.debit" class="input py-1.5 pl-8 text-right text-sm font-mono"
                           @input="line.credit = 0" min="0">
                  </div>
                </td>
                ...
                <td class="px-4 py-2">
                  <div class="relative">
                    <span class="absolute left-3 top-2 text-neutral-400 text-xs">Rp</span>
                    <input type="number" v-model.number="line.credit" class="input py-1.5 pl-8 text-right text-sm font-mono"
                           @input="line.debit = 0" min="0">
                  </div>
                </td>

                <!-- Action -->
                <td class="px-4 py-2 text-center">
                  <button class="text-neutral-400 hover:text-red-500 transition-colors p-1" @click="removeLine(index)" v-if="form.lines.length > 2">
            ...
            <tfoot class="bg-neutral-50/80 font-bold text-neutral-700">
              <tr>
                <td colspan="2" class="px-4 py-3 text-right uppercase text-xs tracking-wider">Total</td>
                <td class="px-4 py-3 text-right font-mono text-sm" :class="isBalanced ? 'text-green-600' : 'text-neutral-800'">
              ...
              <tr v-if="!isBalanced">
                <td colspan="2" class="px-4 py-2 text-right text-xs text-red-500 font-medium">Selisih (Tidak Balance)</td>
                <td colspan="2" class="px-4 py-2 text-center text-red-600 font-mono text-sm bg-red-50">
