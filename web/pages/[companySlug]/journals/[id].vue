  ...
  <NuxtLayout name="dashboard">
      ...
      <div>
        ...
        <h1 class="text-2xl font-bold text-neutral-800 flex items-center gap-2" v-if="journal">
          ...
          <span
            ...
            class="px-2 py-0.5 rounded text-xs bg-red-100 text-red-700 font-medium"
            >Reversed</span
          >
        </h1>
    ...
    <div v-else-if="journal" class="space-y-6 animate-slide-up">
      ...
      <div class="glass-card p-6">
        ...
        <p v-if="journal.reference" class="mt-2 text-sm text-neutral-500">
      ...
      <div class="glass-card overflow-hidden">
        ...
        <table class="w-full">
          ...
          <tfoot class="bg-neutral-50 font-bold text-neutral-700">
            <tr>
              <td colspan="2" class="px-6 py-4 text-right uppercase text-xs tracking-wider">
                Total
              </td>
...
<script setup lang="ts">
  ...
  const handleReverse = async () => {
    if (!confirm('Yakin ingin me-reverse jurnal ini? \nJurnal pembalik akan dibuat otomatis.'))
      return

    try {
      await journalStore.reverseJournal(slug, id)
      toast.success('Berhasil', 'Jurnal berhasil di-reverse!')
      // Refresh data
      journal.value = await journalStore.fetchJournal(slug, id)
    } catch (e: any) {
      toast.apiError(e, 'Gagal Reverse Jurnal')
    }
  }
