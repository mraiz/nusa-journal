  ...
  <div>
    ...
    >
      <div class="w-full sm:w-72">
        <SearchInput v-model="search" placeholder="Search products..." @search="handleSearch" />
      </div>
    </div>
            ...
            <tr v-for="item in products" :key="item.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-6 py-4 text-xs text-slate-500 space-y-1">
                <div v-if="item.salesAccount" class="flex gap-2">
                  <span class="text-green-600 font-medium">Sales:</span>
                  {{ item.salesAccount.name }}
                </div>
                <div v-if="item.purchaseAccount" class="flex gap-2">
                  <span class="text-amber-600 font-medium">Exp:</span>
...
<script setup lang="ts">
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      toast.apiError(err, 'Gagal Menghapus Produk')
    }
  }
</script>
