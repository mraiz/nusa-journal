        ...
        <table class="w-full">
          ...
          <tbody v-if="payments?.length > 0" class="divide-y divide-slate-100">
            <tr v-for="item in payments" :key="item.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td
                class="px-6 py-4 text-sm text-right font-mono font-medium"
    ...
    <div v-if="showCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div class="bg-white rounded-xl shadow-xl w-full max-w-lg relative z-10 p-6">
        ...
        <form @submit.prevent="submit" class="space-y-4">
          ...
          <!-- Allocation Mode -->
          <div>
            <label class="block text-xs font-semibold text-neutral-500 uppercase mb-2"
          ...
          <div v-if="form.allocationMode === 'DIRECT'">
            ...
            <select v-model="form.contraAccountId" required class="input py-2">
              ...
              <option v-for="acc in contraAccounts" :key="acc.id" :value="acc.id">
                {{ acc.code }} - {{ acc.name }}
              </option>
            </select>
          </div>
...
<script setup lang="ts">
  ...
  const handleEdit = async (item: any) => {
    ...
    if (item.journal?.journalLines) {
      const bankLine = item.journal.journalLines.find(
        (l: any) =>
    ...
    if (item.salesInvoiceId) {
      ...
      if (item.journal?.journalLines) {
        const contraLine = item.journal.journalLines.find(
          (l: any) => l.accountId !== form.paymentAccountId
        )
  ...
  const handleDelete = async (id: string) => {
    if (
      !confirm('Are you sure you want to delete this payment? This will reverse the journal entry.')
    )
      return
    try {
      await paymentStore.deletePayment(id)
      toast.success('Berhasil', 'Payment berhasil dihapus')
    } catch (err: any) {
      toast.apiError(err, 'Gagal Menghapus Payment')
    }
  }
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success(
        'Berhasil',
        editingId.value ? 'Payment berhasil diupdate' : 'Payment berhasil dibuat'
      )
    } catch (err: any) {
      toast.apiError(err, 'Gagal Menyimpan Payment')
    } finally {
