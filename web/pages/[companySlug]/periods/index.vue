  ...
  <NuxtLayout name="dashboard">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
      ...
      <div class="flex gap-2 items-center">
        <!-- Year Filter -->
        <div class="w-32">
          <BaseSelect v-model="selectedYear" :options="yearOptions" placeholder="Pilih Tahun" />
        </div>
    ...
    <!-- Content -->
    <div
      v-if="periodStore.loading && periodStore.periods.length === 0"
    ...
    <div
      ...
      class="p-12 text-center text-neutral-500 glass-card"
    >
      <p class="text-lg mb-2">Belum ada periode akuntansi.</p>
      <button class="text-primary-600 hover:underline" @click="openCreateModal">
    ...
    <div v-else class="grid grid-cols-1 gap-4">
      ...
      >
        ...
        <div class="flex gap-3 mt-4 md:mt-0">
          ...
          <button v-else class="btn btn-sm btn-secondary" @click="confirmReopen(period)">
            🔓 Buka Kembali
          </button>
        </div>
      </div>
    ...
    <div
      v-if="showModal"
      class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/30 backdrop-blur-sm animate-fade-in"
    >
...
<script setup lang="ts">
  ...
  const submitCreate = async () => {
    try {
      ...
      toast.apiError(e, 'Gagal Membuat Periode')
    }
  }
  ...
  const confirmClose = async (period: Period) => {
    if (
      confirm(
        `Yakin ingin menutup buku periode "${period.name}"? \nJurnal tidak bisa ditambah/diedit lagi di periode ini.`
      )
    ) {
      try {
        await periodStore.closePeriod(slug, period.id)
        toast.success('Berhasil', `Periode "${period.name}" berhasil ditutup`)
      } catch (e: any) {
        toast.apiError(e, 'Gagal Menutup Periode')
      }
    }
  }
  ...
  const confirmReopen = async (period: Period) => {
    if (confirm(`Yakin ingin membuka kembali periode "${period.name}"?`)) {
      try {
        ...
        toast.success('Berhasil', `Periode "${period.name}" berhasil dibuka kembali`)
      } catch (e: any) {
        toast.apiError(e, 'Gagal Membuka Periode')
      }
    }
  }
