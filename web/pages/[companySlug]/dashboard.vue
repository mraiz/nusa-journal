<template>
  <NuxtLayout name="dashboard">
    ...
    <div class="relative mb-10 p-6 rounded-3xl overflow-hidden animate-slide-down">
      <div
        class="absolute inset-0 bg-gradient-to-r from-primary-500/10 to-accent-500/10 backdrop-blur-3xl"
      ></div>
      <div
        class="absolute -top-24 -right-24 w-64 h-64 bg-primary-400/20 rounded-full blur-3xl"
      ></div>
      <div
        class="absolute -bottom-24 -left-24 w-64 h-64 bg-accent-400/20 rounded-full blur-3xl"
      ></div>

      <div class="relative z-10">
        <h1 class="text-xl md:text-3xl font-bold text-neutral-800 leading-tight">
          Selamat Datang,
          <span
            class="text-transparent bg-clip-text bg-gradient-to-r from-primary-600 to-accent-600 block sm:inline"
            >{{ user?.name?.split(' ')[0] }}</span
          ><span class="hidden sm:inline">! 👋</span>
        </h1>
        <p class="text-neutral-600 mt-2 text-lg">
          Berikut ringkasan performa bisnis
          <span class="font-bold text-neutral-800">{{ currentCompany?.name }}</span>
          <span v-if="periodStore.currentPeriod">
            untuk periode {{ periodStore.currentPeriod.name }}</span
          >.
        </p>
      </div>
    </div>
    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10 animate-fade-in">
        ...
        >
          <div
            class="absolute -right-6 -top-6 w-28 h-28 rounded-full opacity-10 transition-transform duration-500 group-hover:scale-125"
            :class="stat.bgClass"
          ></div>
          ...
          <div class="relative z-10 flex flex-col h-full justify-between">
            <div>
              <div class="flex items-center justify-between mb-4">
                <span
                  class="p-2 rounded-lg bg-neutral-50 border border-neutral-100 text-2xl group-hover:scale-110 transition-transform duration-300"
                  >{{ stat.icon }}</span
                >
                <span
                ...
                >
                  ...
                  <svg class="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path
                      ...
                      stroke-width="2.5"
                      :d="stat.trendUp ? 'M13 7l5 5m0 0l-5 5m5-5H6' : 'M13 17l5-5m0 0l-5-5m5 5H6'"
                      transform="rotate(-45)"
                    />
                  </svg>
                  {{ stat.trend }}
              ...
              <h3 class="text-2xl lg:text-3xl font-bold text-neutral-800 tracking-tight">
                {{ stat.value }}
              </h3>
            </div>
      ...
      <div class="grid lg:grid-cols-3 gap-8 mb-10 animate-slide-up" style="animation-delay: 0.1s">
        ...
        <div class="lg:col-span-2 glass-card p-8 flex flex-col h-[500px]">
          <div class="flex items-center justify-between mb-8">
            <div>
              <h3 class="text-xl font-bold text-neutral-800">
                Arus Kas (Pendapatan vs Pengeluaran)
              </h3>
          ...
          <div class="flex-1 flex items-end justify-between gap-4 px-2 pb-2 relative">
            ...
            <div
              v-for="(item, i) in analyticsSeries"
              :key="i"
              class="relative z-10 flex-1 flex flex-col justify-end items-center group h-full"
            >
              <div
                class="w-full max-w-[40px] flex flex-col justify-end h-[85%] gap-1 hover:scale-105 transition-transform cursor-pointer relative"
              >
                <!-- Tooltip -->
                <div
                  class="opacity-0 group-hover:opacity-100 absolute -top-16 left-1/2 -translate-x-1/2 bg-neutral-900/90 text-white text-[10px] py-1.5 px-3 rounded-lg shadow-xl pointer-events-none transition-opacity whitespace-nowrap z-50 backdrop-blur-sm border border-white/10"
                >
                ...
                <!-- Income Bar -->
                <div
                  class="w-full bg-gradient-to-t from-primary-500 to-primary-400 rounded-t-sm relative shadow-sm"
                  :style="{ height: `${calculateHeight(item.revenue)}%` }"
                ></div>
                <!-- Expense Bar -->
                <div
                  class="w-full bg-gradient-to-t from-red-400 to-red-300 rounded-b-sm opacity-90 shadow-sm"
                  :style="{ height: `${calculateHeight(item.expense)}%` }"
                ></div>
              </div>
        ...
        <div class="space-y-8 flex flex-col">
          ...
          <div class="glass-card p-6 flex-1">
            ...
            <div
              ...
              class="text-center text-neutral-400 text-sm py-8"
            >
              Belum ada aktivitas.
            </div>
            <div v-else class="space-y-4">
              <NuxtLink
                ...
                class="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 group cursor-pointer hover:bg-neutral-50 p-3 rounded-2xl transition-colors -mx-3"
              >
                <!-- Icon -->
                <div
                  class="w-10 h-10 rounded-xl flex items-center justify-center text-xl shadow-sm bg-white border border-neutral-100 text-neutral-500 group-hover:scale-105 transition-transform"
                >
                  📝
                </div>
                ...
                <div class="flex-1 min-w-0">
                  <p class="text-sm font-bold text-neutral-800 truncate mb-0.5">
                    {{ journal.description }}
                  </p>
                  <p class="text-xs text-neutral-500 truncate">
                    {{ journal.journalNumber }} •
                    {{ journal.createdBy?.name?.split(' ')[0] || 'User' }}
                  </p>
                </div>

                <!-- Right Side Info -->
                <div
                  class="flex sm:block justify-between items-center w-full sm:w-auto mt-2 sm:mt-0 text-right"
                >
                  <p class="text-sm font-bold font-mono text-neutral-800">
                    {{ formatCurrency(journal.totalDebit) }}
                  </p>
                  <p class="text-[10px] font-medium text-neutral-400 mt-0 sm:mt-1">
      ...
      <div class="glass-card p-6">
        ...
        <div class="grid grid-cols-2 gap-3">
          ...
          >
            <span
              class="text-2xl group-hover:scale-110 transition-transform duration-300 bg-neutral-50 p-2 rounded-full"
              >✍️</span
            >
