...
<script setup lang="ts">
  ...
  onMounted(async () => {
    await Promise.all([contactStore.fetchVendors(), productStore.fetchProducts()])
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success('Berhasil', 'Bill berhasil diupdate')
      router.push(`/${route.params.companySlug}/purchase/${route.params.id}`)
    } catch (err: any) {
      toast.apiError(err, 'Gagal Update Bill')
    } finally {
      saving.value = false
    }
  }
