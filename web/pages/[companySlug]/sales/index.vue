<template>
  <div>
    <div class="flex items-center justify-between mb-6">
      <h1 class="text-2xl font-bold text-slate-800">Sales Invoices</h1>
      <NuxtLink 
        :to="`/${route.params.companySlug}/sales/create`"
    ...
    <div v-else class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col h-full">
        <div class="overflow-x-auto">
            <table class="w-full">
                ...
                <tbody v-if="invoices?.length > 0" class="divide-y divide-slate-100">
                    <tr v-for="item in invoices" :key="item.id" class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 text-sm text-slate-500">
                            {{ new Date(item.date).toLocaleDateString('id-ID') }}
                        </td>
                        <td class="px-6 py-4 text-sm font-medium text-slate-900">
                            <NuxtLink :to="`/${route.params.companySlug}/sales/${item.id}`" class="hover:underline text-primary-600">
                                {{ item.invoiceNumber }}
                            </NuxtLink>
                        </td>
        ...
        <div v-if="invoices?.length === 0 && !loading">
            <EmptyState
                title="No Invoices Yet"
                description="Create your first invoice to start tracking your sales and revenue."
                icon="💰"
                action-label="Create Invoice"
                @action="router.push(`/${route.params.companySlug}/sales/create`)"
            />
        </div>
...
const invoices = computed(() => salesStore.invoices)
