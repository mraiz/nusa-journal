      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div>
             ...
             <div>
                <label class="text-xs font-semibold text-neutral-500 uppercase mb-1">Invoice Number</label>
                <input v-model="form.invoiceNumber" type="text" required class="input py-2" placeholder="INV-2026-001">
            </div>
             <div>
...
import { useContactStore, useProductStore, useSalesStore } from '@/stores'
...
onMounted(async () => {
    ...
    // Fetch invoice data
    const invoice = await salesStore.fetchInvoice(route.params.id as string)
    if (invoice) {
        form.customerId = invoice.customerId
        form.invoiceNumber = invoice.invoiceNumber
        form.date = new Date(invoice.date).toISOString().split('T')[0]
        form.dueDate = invoice.dueDate ? new Date(invoice.dueDate).toISOString().split('T')[0] : ''
        form.notes = invoice.notes || ''
        form.lines = invoice.lines.map((l: any) => ({
...
const submit = async () => {
    saving.value = true
    try {
        await salesStore.updateInvoice(route.params.id as string, {
            ...form,
            date: new Date(form.date),
            dueDate: form.dueDate ? new Date(form.dueDate) : undefined
        })
        router.push(`/${route.params.companySlug}/sales/${route.params.id}`)
    } catch (err: any) {
