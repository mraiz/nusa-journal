...
<script setup lang="ts">
  ...
  onMounted(async () => {
    await Promise.all([contactStore.fetchCustomers(), productStore.fetchProducts()])
  ...
  const submit = async () => {
    ...
    try {
      ...
      toast.success('Berhasil', 'Invoice berhasil diupdate')
      router.push(`/${route.params.companySlug}/sales/${route.params.id}`)
    } catch (err: any) {
      toast.apiError(err, 'Gagal Update Invoice')
    } finally {
      saving.value = false
    }
  }
