<template>
  <div class="min-h-screen bg-slate-50 relative overflow-hidden">
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none">
      <div
        class="absolute -top-20 -right-20 w-96 h-96 bg-primary-200/20 rounded-full blur-3xl animate-float"
      ></div>
      <div
        class="absolute top-1/2 -left-20 w-80 h-80 bg-accent-200/20 rounded-full blur-3xl animate-float"
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      <div class="flex items-center justify-between mb-10 animate-slide-down">
        ...
        <div class="flex items-center gap-4">
          <div
            class="flex items-center gap-2 px-4 py-2 bg-white rounded-full shadow-sm border border-neutral-200"
          >
            <div
              class="w-8 h-8 bg-gradient-to-br from-primary-500 to-primary-600 rounded-full flex items-center justify-center text-white text-sm font-bold shadow-soft"
            >
              {{ userInitials }}
            </div>
            ...
            <button
              @click="handleLogout"
              class="ml-2 text-neutral-400 hover:text-red-500 transition-colors"
              title="Logout"
            >
      ...
      <div v-else-if="companies.length === 0" class="glass-card p-12 text-center animate-scale-in">
        <div
          class="w-20 h-20 bg-gradient-to-br from-primary-100 to-accent-100 rounded-full flex items-center justify-center mx-auto mb-6 shadow-glow-blue"
        >
        ...
        <p class="text-neutral-600 mb-8 max-w-md mx-auto">
          Anda belum terhubung dengan perusahaan manapun. Buat perusahaan baru untuk mulai mencatat
          keuangan.
        </p>
      ...
      <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-fade-in">
        ...
        >
          <div
            class="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-soft mb-4 group-hover:scale-110 transition-transform"
          >
            <svg
          ...
          <h3 class="font-semibold text-neutral-800 group-hover:text-primary-600">
            Buat Perusahaan Baru
          </h3>
        ...
        <button
          @click="openJoinModal"
          class="glass-card p-6 flex flex-col items-center justify-center text-center border-dashed border-2 border-neutral-300 hover:border-accent-400 hover:bg-accent-50/50 group transition-all duration-300 min-h-[220px]"
        >
          <div
            class="w-14 h-14 bg-white rounded-full flex items-center justify-center shadow-soft mb-4 group-hover:scale-110 transition-transform"
          >
          ...
          <h3 class="font-semibold text-neutral-800 group-hover:text-accent-600">
            Gabung Perusahaan
          </h3>
          <p class="text-sm text-neutral-500 mt-1">Masuk dengan ID Perusahaan</p>
        </button>
        ...
        >
          <div>
            <div class="flex items-start justify-between mb-4">
              <div
                class="w-12 h-12 bg-gradient-to-br from-primary-500 to-accent-500 rounded-xl flex items-center justify-center text-white text-lg font-bold shadow-soft group-hover:scale-105 transition-transform"
              >
                {{ getInitials(company.name) }}
              </div>
            </div>
            <h3
              class="text-lg font-bold text-neutral-800 mb-1 group-hover:text-primary-600 transition-colors"
            >
              {{ company.name }}
            </h3>
          ...
          <div
            class="mt-6 pt-4 border-t border-neutral-100 flex items-center justify-between text-sm"
          >
            ...
            <span
              v-else
              class="text-primary-600 font-medium group-hover:translate-x-1 transition-transform inline-flex items-center gap-1"
            >
              Dashboard
