    ...
    >
      ...
      <div class="h-16 flex items-center px-6 border-b border-neutral-100">
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-gradient-to-br from-primary-500 to-accent-500 rounded-lg flex items-center justify-center text-white text-sm font-bold shadow-soft">
            NJ
          </div>
      ...
      <!-- Company Switcher -->
      <div class="p-4">
        <div class="p-3 bg-primary-50 rounded-xl border border-primary-100 flex items-center gap-3 cursor-pointer hover:bg-primary-100 transition-colors">
          <div class="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-primary-600 font-bold shadow-sm">
            {{ companyInitials }}
          </div>
          <div class="flex-1 min-w-0">
            <p class="text-sm font-bold text-neutral-800 truncate">{{ currentCompany?.name }}</p>
            <p class="text-xs text-neutral-500 truncate">{{ currentCompany?.plan || 'Free Plan' }}</p>
          </div>
        ...
        <div v-for="(group, index) in menuGroups" :key="index" class="mb-6">
          ...
          <div class="space-y-1">
            <NuxtLink 
              ...
              :class="$route.path.includes(item.path) && item.path !== '' ? 'bg-primary-50 text-primary-700 shadow-sm' : 'text-neutral-600 hover:bg-neutral-50 hover:text-neutral-900'"
            >
      ...
      <!-- User Profile -->
      <div class="absolute bottom-0 left-0 w-full p-4 border-t border-neutral-100 bg-white/50 backdrop-blur-sm">
        <div class="flex items-center gap-3">
          <div class="w-9 h-9 bg-neutral-200 rounded-full flex items-center justify-center text-neutral-600 font-bold overflow-hidden">
          ...
          <button @click="handleLogout" class="text-neutral-400 hover:text-red-500 transition-colors">
